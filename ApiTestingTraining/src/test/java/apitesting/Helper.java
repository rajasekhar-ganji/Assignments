...
public class Helper {
		...
		public void OAuth1() {
			Response response=RestAssured.given().baseUri("https://dummy.restapiexample.com/api/v1").auth().oauth("ASDFGHJJJKKK", "QWETTYYY", "GHHHHIIKK", "ZXCVBNMMNNN").when().get("/employees").then().statusCode(200).log().all().extract().response();
			System.out.println(response.getBody().asString());
			System.out.println(response.getStatusCode());
